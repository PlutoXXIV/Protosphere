...
// Prints the phone message to Serial Monitor.
// If an LCD is present, also displays the first 16 characters on it.
void lcdShowMessage(const String &msg) {

  // Print the complete message received from the phone
  Serial.print("Message from phone: ");
  Serial.println(msg);

  // No LCD? Stop here, but the message has already been printed.
  if (!lcdFound) return;

  // LCD can display only 16 characters on the second line
...
void handleSensor() {
  long d = getDistance();
  server.send(200, "text/plain", d >= 0 ? String(d) : "out of range");
}
...
void setup() {
  ...
  if (lcdFound) {
    ...
    lcdSetCursor(0, 1);
    lcdPrint("192.168.4.1");

    Serial.println("ESP32_AP   192.168.4.1");
  }
  else {
